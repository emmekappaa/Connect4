...
int main(int argc, char* argv[])
{
    ...
    //NB: controlliamo con la funzione checkInput
    ...
    array_pid[0] = getpid();// Set dimensioni tabellone in shared
    ...
    // Avviso che il campo da gioco è stato impostato
    ...
    {
        // Errno relativo al interrupt della CTRL-C 
        if(errno == 4){
            if (semop(sem_id2, &wait2_op, 1) == -1){
